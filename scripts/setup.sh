...
step_cron() {
  ...
  tz="$(ask_default "  Timezone" "$(jq -r '.timezone // "Europe/Rome"' "$(settings_file)" 2>/dev/null || printf 'Europe/Rome')")"
  echo "  Defaults target US engagement windows (9:30/12:30/15:00 ET from Europe)."
  t1="$(ask_hhmm "  Slot 1 (own work) HH:MM" "15:30")"
  t2="$(ask_hhmm "  Slot 2 (AI tools/news) HH:MM" "18:30")"
  t3="$(ask_hhmm "  Slot 3 (aviation) HH:MM" "21:00")"
